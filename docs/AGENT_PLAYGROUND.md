...
| `psycopg` or `psycopg2-binary` | latest | `python3 -c "import psycopg"` | Required only when `NIGHTPAY_DB_BACKEND=postgres` |
...
| `MIP003_MAX_INFLIGHT` | `512` | Max in-flight HTTP requests per MIP-003 process (backpressure guard) |
| `NIGHTPAY_DB_BACKEND` | `sqlite` | `sqlite` for local/single-node, `postgres` for horizontal API scaling |
| `NIGHTPAY_DATABASE_URL` | _(empty)_ | Postgres DSN (required when `NIGHTPAY_DB_BACKEND=postgres`) |
| `NIGHTPAY_DB_POOL_SIZE` | `64` | Max Postgres connections per MIP-003 process |
...
For horizontal API scale, set `NIGHTPAY_DB_BACKEND=postgres`, provide a shared `NIGHTPAY_DATABASE_URL`, and run multiple `mip003-server.sh` instances behind your reverse proxy/load balancer (same secrets on every instance).
