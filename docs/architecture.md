...
| **mip003-server.sh** | Exposes MIP-003 job endpoints (start, claim, submit, vote, select winner, status) plus public ontology routes (`/ontology`, `/ontology/context`, `/ontology/examples`). Storage backend is `sqlite` by default, or shared `postgres` for horizontal API scaling. |
