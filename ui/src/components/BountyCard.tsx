...
import type { ReadinessBand } from '../utils/boardSearch.ts';
...
export default function BountyCard({
  ...
  const assignedAgentPath = bounty.assignedAgentId ? `/agents/${encodeURIComponent(bounty.assignedAgentId)}` : null;
  ...
  const hasLongDescription = description.length > DESCRIPTION_PREVIEW_CHARS;
  ...
  const readinessTone =
    readinessBand === 'high'
      ...
      : readinessBand === 'medium'
        ...
        : 'border-void-600/80 bg-void-800/70 text-gray-300';
    ...
    <article className={`card card-elevated group relative ${compact ? 'p-4 sm:p-5' : 'p-5 sm:p-6'} transition-all duration-300 hover:-translate-y-[2px] hover:shadow-[0_15px_30px_-5px_rgba(0,0,0,0.6),0_0_20px_rgba(94,242,255,0.15)] ${isMyClaim ? 'ring-1 ring-neon-cyan/70 shadow-[0_0_15px_rgba(94,242,255,0.1)]' : ''}`}>
      ...
      <div className="mb-4 flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <h2 className={`${compact ? 'text-[16px]' : 'text-[20px]'} break-words font-bold leading-[1.22] text-gray-100 transition-colors`} title={bounty.title}>{bounty.title}</h2>
      ...
      <p
        title={!descriptionExpanded ? description : undefined}
        className={`${compact ? 'mb-2 text-[13px] leading-6' : 'mb-3 text-[15px] leading-7'} whitespace-pre-wrap break-words text-gray-300 transition-colors`}
      >
        {visibleDescription}
      </p>
      {(typeof readinessScore === 'number' || tags.length > 0) && (
        <div className="mb-3.5 flex flex-wrap items-center gap-2">
          {typeof readinessScore === 'number' && (
            <span className={`rounded-full border px-2 py-1 text-[11px] font-bold uppercase tracking-[0.06em] ${readinessTone}`}>
              Agent ready {Math.round(readinessScore)}/100
            </span>
          )}
          {tags.slice(0, compact ? 4 : 6).map((tag) => {
            if (!onTagClick) {
              return (
                <span
                  key={`${bounty.id}-${tag}`}
                  className="rounded-full border border-neon-cyan/25 bg-neon-cyan/10 px-2.5 py-1 text-[11px] font-semibold text-neon-cyan"
                >
                  {tag}
                </span>
              );
      ...
      {hasLongDescription && (
        <button
          ...
          className="mb-4 text-[11px] font-semibold uppercase tracking-wider text-neon-cyan transition-colors hover:text-white"
        >
      ...
      <div className={`grid gap-2 rounded-[14px] border border-void-700/60 bg-void-800/40 text-xs text-gray-400 shadow-inner ${compact ? 'p-3 sm:grid-cols-3' : 'p-4 sm:grid-cols-3'}`}>
        ...
        <div className="flex flex-col gap-1.5 border-r border-void-700/40 px-2 sm:px-4">
          ...
          {assignedAgentPath && bounty.claimsCount > 0 ? (
            <Link to={assignedAgentPath} className="font-semibold text-neon-cyan text-[13px] hover:underline">
              {claimedLabel}
            </Link>
          ) : (
