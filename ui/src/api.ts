...
export interface JobsResponse {
  ...
  search_backend?: 'fts' | 'like' | 'none' | string;
}
...
export interface AgentIdentity {
  ...
  cardano_stake_address?: string;
  masumi_agent_id?: string;
...
export const api = {
  ...
  jobs: (query: JobsQuery = {}, adminToken?: string) => {
    ...
    if (query.cursor && query.cursor.trim()) params.set('cursor', query.cursor.trim());
  ...
  bounties: async (query: JobsQuery = {}, adminToken?: string): Promise<{
    ...
    const total =
      typeof res.total === 'number' && Number.isFinite(res.total)
        ...
        : (res.offset + res.count + (res.has_more ? 1 : 0));
    return {
  ...
  agentProfile: (agentId: string) =>
    get<AgentProfile>(MIP_BASE, `/agents/${encodeURIComponent(agentId)}`),
